...
#include "caffe/layers/gfocal_loss_layer.hpp"

namespace caffe {

template <typename Dtype>
void GFocalLossLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void GFocalLossLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void GFocalLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void GFocalLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
//		focal_diff = alphat * pow(1 - pt, gamma_) * (gamma_ * pt * log(pt) + pt - 1);
	  } else {
		pc = prob_data[j];
		alphat = 1 - alpha_;
...
#ifdef CPU_ONLY
STUB_GPU(GFocalLossLayer);
#endif

INSTANTIATE_CLASS(GFocalLossLayer);
REGISTER_LAYER_CLASS(GFocalLoss);
